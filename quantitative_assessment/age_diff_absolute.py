...
def get_image_to_age_mapping():
    image_to_age_map = {}
    metadata = pd.read_pickle(MEATADATA_PATH)
    for i in range(len(metadata['photo_taken_age'])):
        age = metadata['photo_taken_age'][i]
        filename = metadata['full_path'][i].split('/')[1].split('.')[0]
        image_to_age_map[filename] = float(age)
    return image_to_age_map
...
def main():
    ...
    for img, filename in image_generator:
        ...
        if len(detected) > 0:
            ...
            image_name = filename.split('/')[-1].split('.')[0].replace(TARGET_SUFFIX, '')
    ...
    print('Neg Age Progression: {:d} out of {:d}'.format(negative_count, len(age_diffs)))
